...
class DashboardPage extends JPanel {
    ...
    private void onCalculate() {
        ...
        double costPrice;
        try { costPrice = Double.parseDouble(tfCost.getText().trim()); }
        catch (Exception e) { err("Enter a valid cost price.");    return; }
        
        double profitMargin;
        try { profitMargin = Double.parseDouble(tfMargin.getText().trim()); }
        catch (Exception e) { err("Enter a valid profit margin."); return; }
        
        double[] competitors;
        try {
            String[] parts = tfCompetitors.getText().split(",");
            competitors = new double[parts.length];
            for (int i = 0; i < parts.length; i++)
                competitors[i] = Double.parseDouble(parts[i].trim());
        } catch (Exception e) { err("Competitor prices format: 1100, 1200, 1050"); return; }

        // Calculate average competitor price
        double avgCompetitor = 0;
        for (double cp : competitors) avgCompetitor += cp;
        avgCompetitor /= competitors.length;
        avgCompetitor = Math.round(avgCompetitor * 100.0) / 100.0;

        // Calculate minimum price (with profit margin protection)
        double minPrice = costPrice + (costPrice * profitMargin / 100);
        minPrice = Math.round(minPrice * 100.0) / 100.0;

        // Get demand level and season
